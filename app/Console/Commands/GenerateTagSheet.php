...
use App\Packages\TagSheetUpload\SheetRow;
use Carbon\Carbon;
...
class GenerateTagSheet extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tagsheet:generate';
     ...
     * @var string
     */
    protected $description = 'Generate the tag sheet and upload to google drive.';
    ...
    {
        $filename = sys_get_temp_dir() . '/' . Carbon::now()->format('dmyHis') . 'tagsheet_generation.csv';
        $csv = Writer::createFromPath($filename, 'w+');
        $csv->insertAll($data);

        // Get the current file so we can overwrite it later
        $path = config('app.tag_sheet_drive_id');
